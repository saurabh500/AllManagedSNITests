...
namespace AllManagedSNITests
{
    class _085
    {
                ...
                try
                {
                    ...
                    using (new Timer(TimerCallback, state: null, dueTime: TimeSpan.FromSeconds(5), period: TimeSpan.FromSeconds(5)))
                    {
                        results = Parallel.For(
                            fromInclusive: 0,
                            toExclusive: iterationCount,
                            (int i) => DoManyAsync(connString).GetAwaiter().GetResult());
                    }
                }
                catch (Exception ex)
                {
        ...
        private static void DisplaySummary()
        {
            ...
            Console.WriteLine($"{_watch.Elapsed} {_continue} Started:{_start} Done:{_done} InFlight:{_inFlight} RowsRead:{_rowsRead} ResultRead:{_resultRead} PoisonedEnded:{_poisonedEnded} nonPoisonedExceptions:{_nonPoisonedExceptions} PoisonedCleanupExceptions:{_poisonCleanUpExceptions} Count:{count} Found:{_found}");
        }
        ...
        private static int _resultRead;
        private static int _nonPoisonedExceptions;
        private static int _poisonedEnded;
        private static int _poisonCleanUpExceptions;
        private static bool _continue = true;
        private static int _found;
        private static Random _random;
        private static object _lockObject = new object();

        private static HashSet<string> _exceptionDetails = new HashSet<string>();
    }
}
